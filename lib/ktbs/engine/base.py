...
class Base(WithLockMixin, BaseMixin, KtbsPostableMixin, KtbsResource):
                   ...
                   WHERE {
                       {
                           ...
                           # handle exception generated by the simple JSON parser
                            <%s> ktbs:hasBase ?c .
                            ?c a ktbs:Base
                       }
                   }
        """ % (KTBS, self.uri, self.uri)
    ...
    def check_posted_graph(self, parameters, created, new_graph):
        ...
        """
        diag = super().check_posted_graph(parameters, created, new_graph)

        # The simple JSON parser uses "inRoot" (inverse of k:hasBase)
        # to contextualize a new base,
        # but when the container is another base, it should be "inBase" (inverse of k:contains).
        #
        # Since the simple JSON parser does not have enough info to do it properly,
        # we "fix" it here.
        has_base_triple = (self.uri, KTBS.hasBase, created)
        new_graph.remove(has_base_triple)
        new_graph.add((self.uri, KTBS.contains, created)) # may alteady be here, but just in case
